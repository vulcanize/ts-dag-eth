...
const rctTrieCodec = codecs[name]
const { assert } = chai
const test = it
...
describe('eth-trie', function () {
  ...
  const expectedExtension = packTwoMemberNode(rctTrieCodec.code, extensionNodeBuffer as any) as TrieNode
  ...
  const expectedLeaf = packTwoMemberNode(rctTrieCodec.code, leafNodeBuffer as any) as TrieNode
  ...
  const expectedBranchNode = packBranchNode(rctTrieCodec.code, branchNodeBuffer as any) as TrieNode
  ...
  test('encode and decode round trip', () => {
    const extensionNode: TrieNode = rctTrieCodec.decode(extensionNodeRLP)
    same(extensionNode, expectedExtensionNode)
    const extensionNodeEnc = rctTrieCodec.encode(extensionNode)
    same(extensionNodeEnc, extensionNodeRLP)

    const leafNode: TrieNode = rctTrieCodec.decode(leafNodeRLP)
    same(leafNode, expectedLeaf)
    const leafNodeEnc = rctTrieCodec.encode(leafNode)
    same(leafNodeEnc, leafNodeRLP)

    const branchNode: TrieNode = rctTrieCodec.decode(branchNodeRLP)
    same(branchNode, expectedBranchNode)
    const branchNodeEnc = rctTrieCodec.encode(branchNode)
